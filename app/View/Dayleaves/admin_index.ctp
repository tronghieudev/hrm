	...
	<table class="table table-bordered table-hover">
		...
		<tbody>
		    <?php foreach($data as $item) { ?>
	            <tr id="tr<?php echo $item['UsersDaysLeave']['id']; ?>">
	            	<td></td>
	                <td><?= $item['User']['email']; ?></td>
	                <td>
	                	<?= date('d-m-Y', strtotime($item['UsersDaysLeave']['day_start'])); ?>
	                </td>
	                <td>
	                	<?= $item['UsersDaysLeave']['days'] ?>
	                	ngày
	                </td>
	                <td>
	                	<span class="glyphicon glyphicon-ok ok" data-id="<?php echo $item['UsersDaysLeave']['id']; ?>" style="color: green; cursor: pointer;"></span>
	                </td>
	            </tr>
			<?php } ?>
		</tbody>
	</table>
    <?php if($this->params['paging']['UsersDaysLeave']['pageCount'] > 1) { ?>
        <ul class="pagination">
            <?php
